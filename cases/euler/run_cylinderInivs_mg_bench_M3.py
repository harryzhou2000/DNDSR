"""Run the Mach-3 inviscid-cylinder multigrid convergence sweep.

Invoke this legacy driver from the DNDSR build directory. It launches each
configured case immediately with 16 MPI ranks, so review ``opts`` and
``mg_seqs`` before running it locally or through a cluster allocation.
"""
...
out_base = "../data/out/CylinderInvisHalfA-MGtest_2_VRes-M3.dir"
        ...
        options.extend(
            [
                ...
                (
                    ...
                    "[1,1.0,0.0,0,0.6984126984126985]",
                ),
                ("/implicitCFLControl/CFL", 2),
                ("/timeMarchControl/rhsFPPMode", 2),
                ("/eulerSettings/rsRotateScheme", 1),
                ("/limiterControl/useLimiter", "true"),
            ]
        )
